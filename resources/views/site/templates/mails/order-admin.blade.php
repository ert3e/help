...
<div>На сайте «{{ setting('main.title') }}» был сделан заказ.</div>
<br>
...
<div><strong>Имя получателя:</strong> {{ $order->name }}</div>
<div><strong>Контактный телефон:</strong> {{ $order->telephone }}</div>
@if ($order->email)
    <div><strong>Почта получателя:</strong> {{ $order->email }}</div>
@endif
<div><strong>Город получателя:</strong> {{ $order->city }}</div>
<div><strong>Адрес получателя:</strong> {{ $order->address }}</div>
...
<br>
<br>
Подробную информацию о заказе вы можете посмотреть через административную панель сайта или перейдя по ссылке
<a href="{{ URL::to('/admin/orders/'.$order->id) }}" target="_blank">просмотреть заказ №{{ $order->id }}</a>.
